...
struct RatiosTileContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Header row
            GrowthTableHeader(headerText: "Ratios")

            Divider()

            // Profit Margin
            GrowthTableRow(label: "Profit Margin", metrics: overview.ratios.profitMargin)

            Divider()

            // Debt/Equity
            GrowthTableRow(label: "Debt/Equity", metrics: overview.ratios.debtToEquity)

            Divider()

            // Liability/Equity
            GrowthTableRow(label: "Liability/Equity", metrics: overview.ratios.liabilityToEquity)

            Divider()

            // Current Ratio
            GrowthTableRow(label: "Current Ratio", metrics: overview.ratios.currentRatio)

            Divider()

            // Quick Ratio
            GrowthTableRow(label: "Quick Ratio", metrics: overview.ratios.quickRatio)
        }
    }
}
